...
public class PlanDetailViewPagerAdapter extends RecyclerView.Adapter<PlanDetailViewPagerAdapter.PlanDetailViewHolder> {
    ...
    public void onBindViewHolder(@NonNull PlanDetailViewPagerAdapter.PlanDetailViewHolder holder, int position) {
        ...
        Integer total = arrayList.get(position).getTotal();
        ...
        //리사이클러뷰 어댑터 설정
        PlanDetailAdapter planDetailAdapter = new PlanDetailAdapter(arrayList.get(position).getData());
            ...
            builder.setItems(R.array.edit_delete, ((dialog, which) -> {
                if(which == 0) { //수정
                    ...
                    switch (viewPagerCase) {
                        case BUDGET:
                            intent = new Intent(v.getContext(), InputBudgetActivity.class);
                            v.getContext().startActivity(intent);
                            break;
                        case EXPENSE:
                            intent = new Intent(v.getContext(), InputExpenseActivity.class);
                            v.getContext().startActivity(intent);
                            break;
                    }
                } else { //삭제
